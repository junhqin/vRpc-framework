...
import com.rpc.registry.RegistryServiceCache;
...
public class ZooKeeperRegistry implements Registry {
    ...
    public List<ServiceMetaInfo> serviceDiscovery(String serviceKey) {
        ...
        try {
            ...
            serviceMetaInfoList.forEach(
                    serviceMetaInfo -> {
                        log.info("监听节点："+serviceMetaInfo.getServiceNodeKey());
    ...
    // 其实不需要心跳机制，建立了临时节点，如果服务器故障，则临时节点直接丢失
    @Override
    public void heartBeat() {
    }

    /**
     * 消费端监听服务节点是否下线，若变动则删除本地缓存
    ...
    public void watch(String serviceNodeKey) {
        ...
        if(newWatch){
            log.info("Adding watch on key: {}", watchKey);
            CuratorCache curatorCache = CuratorCache.build(zkClient, watchKey);
            curatorCache.start();
            curatorCache.listenable().addListener(
                CuratorCacheListener
                        .builder()
                        .forDeletes(childData -> {
                            log.info("Node deleted: {}", watchKey);
                            registryServiceCache.clearCache(serviceNodeKey);
                        })
                        .forChanges((oldNode, node) -> {
                            log.info("Node changed: {}", watchKey);
                            registryServiceCache.clearCache(serviceNodeKey);
                        })
                        .build()
            );
        } else {
            log.info("Watch already exists for key: {}", watchKey);
        }
    }
    ...
    public void destroy() {
        ...
        for (String key : localRegisterNodeKeySet) {
            try {
                zkClient.delete().guaranteed().forPath(key);
                log.info("当前服务下线："+ key);
            } catch (Exception e) {
